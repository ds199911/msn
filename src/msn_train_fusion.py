...
def main(args, medfuse_params=None):
    ...
    (unsupervised_loader,
     unsupervised_sampler) = init_fusion_data(
         transform=transform,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, (x, cxr, _, _, seq_length, _) in enumerate(unsupervised_loader):
            ...
            def train_step():
                ...
                # logger.info('shape h' +str(h.shape))
                # logger.info('shape z' +str(z.shape))
...
def load_checkpoint(
    ...
    try:
        ...
        logger.info('failed to loaded prototypes; using init prototypes')
    ...
    try:
        ...
        opt = None
        logger.info(f'failed loaded optimizers from epoch {epoch} using init optimzers instead')
        logger.info(f'read-path: {r_path}')
    del checkpoint
